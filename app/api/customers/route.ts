...
const ensureTable = async (sql: ReturnType<typeof database>) => { await sql`CREATE TABLE IF NOT EXISTS customers (id TEXT PRIMARY KEY,business TEXT NOT NULL,contact TEXT NOT NULL,email TEXT NOT NULL,phone TEXT NOT NULL,address TEXT NOT NULL,access_code TEXT,code_expires_at TIMESTAMPTZ,updated_at BIGINT NOT NULL)`; };

export async function GET(request: Request) {
  if (!await requireSalesUser(request)) return Response.json({ error: "Unauthorized" }, { status: 401 });
  try { const sql = database(); await ensureTable(sql); const customers = await sql`SELECT id,business,contact,email,phone,address,access_code AS "accessCode",code_expires_at AS "codeExpiresAt" FROM customers ORDER BY business`; return Response.json({ customers }); }
...
export async function DELETE(request: Request) {
  ...
  try { const sql = database(); await ensureTable(sql); await sql`DELETE FROM customers WHERE id = ${id}`; return Response.json({ deleted: id }); }
