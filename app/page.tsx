...
function Products({ products, setProducts, filtered, query, setQuery, categories, category, setCategory, setEditing, importCatalog, notify, go }: { products: Product[]; setProducts: (p: Product[]) => void; filtered: Product[]; query: string; setQuery: (s: string) => void; categories: string[]; category: string; setCategory: (s: string) => void; setEditing: (p: Product | null) => void; importCatalog: (e: ChangeEvent<HTMLInputElement>) => void; notify: (s: string) => void; go: (v: View) => void }) {
  ...
  return <div className="page">
    <div className="page-head"><div><p className="eyebrow">Catalog</p><h1>Products & inventory</h1><p>Manage availability, pricing, stock, and customer visibility.</p></div><div className="actions"><label className="button secondary file-button">⇧ Import PDF or CSV<input type="file" accept=".csv,.pdf,text/csv,application/pdf" onChange={importCatalog} /></label><button className="button secondary" onClick={() => notify("Current catalog export prepared")}>⇩ Export</button><button className="button danger-outline" disabled={!products.length} onClick={clearProducts}>Clear all products</button><button className="button primary" onClick={addProduct}>＋ Add product</button></div></div>
