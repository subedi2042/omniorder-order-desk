...
async function createProformaPdf(products: Product[], cart: Cart, quoteLines: QuoteLine[], discountPercent: number, shippingAmount: number, taxPercent: number, quoteNotes: string, approved: boolean) {
...
async function downloadApprovedProformaPdf(products: Product[], cart: Cart, quoteLines: QuoteLine[], discountPercent: number, shippingAmount: number, taxPercent: number, quoteNotes: string) {
  const url = await createProformaPdf(products, cart, quoteLines, discountPercent, shippingAmount, taxPercent, quoteNotes, true);
...
export default function Home() {
  ...
  if (view === "catalog") return <CustomerCatalog {...{ products: customerProducts, pdfProducts: products, query, setQuery, categories, category, setCategory, cart, setQty, cartItems: customerCartItems, reviewOpen, setReviewOpen, orderCreated, setOrderCreated, stage, setStage, go, notify, targetedList, changeRequest, setChangeRequest, quoteLines, discountPercent, shippingAmount, taxPercent, quoteNotes, proformaTotal: stage === "proforma" ? quotedTotal : undefined }} />;
  ...
  return (
    <div className="app-shell">
      ...
      <main className="admin-main">
        ...
        {view === "documents" && (orderCreated ? <Documents stage={stage} setStage={setStage} products={products} cart={cart} quoteLines={quoteLines} discountPercent={discountPercent} shippingAmount={shippingAmount} taxPercent={taxPercent} quoteNotes={quoteNotes} go={go} notify={notify} /> : <EmptyState title="No estimates yet" description="Create an estimate after a customer submits an order request." action="View orders" onAction={() => go("orders")} />)}
...
function Documents({ stage, setStage, products, cart, quoteLines, discountPercent, shippingAmount, taxPercent, quoteNotes, go, notify }: { stage: Stage; setStage: (s: Stage) => void; products: Product[]; cart: Cart; quoteLines: QuoteLine[]; discountPercent: number; shippingAmount: number; taxPercent: number; quoteNotes: string; go: (v: View) => void; notify: (s: string) => void }) {
  const { lines, subtotal, discount, shipping, tax, total } = quoteAmounts(products, cart, quoteLines, discountPercent, shippingAmount, taxPercent);
  const approved = stage === "approved";
  const download = () => { downloadApprovedProformaPdf(products, cart, quoteLines, discountPercent, shippingAmount, taxPercent, quoteNotes); notify("Approved estimate PDF downloaded"); };
...
function CustomerCatalog(props: any) {
  const { products, pdfProducts, query, setQuery, categories, category, setCategory, cart, setQty, cartItems, reviewOpen, setReviewOpen, orderCreated, setOrderCreated, stage, setStage, go, notify, targetedList, changeRequest, setChangeRequest, quoteLines, discountPercent, shippingAmount, taxPercent, quoteNotes, proformaTotal } = props;
  if (stage === "proforma") return <CustomerDocument total={proformaTotal} products={pdfProducts} cart={cart} quoteLines={quoteLines} discountPercent={discountPercent} shippingAmount={shippingAmount} taxPercent={taxPercent} quoteNotes={quoteNotes} setStage={setStage} setChangeRequest={setChangeRequest} go={go} notify={notify} />;
...
function CustomerDocument({ total, products, cart, quoteLines, discountPercent, shippingAmount, taxPercent, quoteNotes, setStage, setChangeRequest, go, notify }: { total: number; products: Product[]; cart: Cart; quoteLines: QuoteLine[]; discountPercent: number; shippingAmount: number; taxPercent: number; quoteNotes: string; setStage: (s: Stage) => void; setChangeRequest: (s: string) => void; go: (v: View) => void; notify: (s: string) => void }) {
  ...
  const openPreview = async () => {
    closePreview();
    setPreviewUrl(await createProformaPdf(products, cart, quoteLines, discountPercent, shippingAmount, taxPercent, quoteNotes, false));
  };
  const submitRequest = () => {
