...
function Landing({ onSales, onCustomer, onPrivacy, onSupport, onFastQuote, signInOpen, customerSignInOpen, onClose, onSignedIn, onCustomerSignedIn }: { onSales: () => void; onCustomer: () => void; onPrivacy: () => void; onSupport: () => void; onFastQuote: () => void; signInOpen: boolean; customerSignInOpen: boolean; onClose: () => void; onSignedIn: (user: SalesUser) => void; onCustomerSignedIn: () => void }) {
  ...
  const verifyCode = () => {
    setCodeError("No active catalog is attached to this code. Ask sales to send a new secure order link.");
  };
...
function CustomerRep() { return <span className="rep-contact"><span className="rep-dot">DK</span><span>Desi Kitchen <b>Sales</b></span><a href="tel:+15624707400">(562) 470-7400</a></span>; }
...
function EmptyState({ title, description, action, onAction }: { title: string; description: string; action: string; onAction: () => void }) { return <div className="page"><div className="page-head"><div><p className="eyebrow">Sales workspace</p><h1>{title}</h1><p>{description}</p></div></div><section className="panel empty"><span>DK</span><h2>{title}</h2><p>{description}</p><button className="button primary" onClick={onAction}>{action} →</button></section></div>; }
