...
export default function Home() {
  ...
  if (view === "catalog") return <CustomerCatalog {...{ products: customerProducts, pdfProducts: products, query, setQuery, categories, category, setCategory, cart, setQty, cartItems: customerCartItems, reviewOpen, setReviewOpen, orderCreated, setOrderCreated, stage, setStage, go, notify, targetedList, proformaTotal: stage === "proforma" ? total || 486.38 : undefined }} />;
...
function CustomerCatalog(props: any) {
  const { products, pdfProducts, query, setQuery, categories, category, setCategory, cart, setQty, cartItems, reviewOpen, setReviewOpen, orderCreated, setOrderCreated, stage, setStage, go, notify, targetedList, proformaTotal } = props;
  if (stage === "proforma") return <CustomerDocument total={proformaTotal} products={pdfProducts} cart={cart} setStage={setStage} go={go} notify={notify} />;
