...
export default function Home() {
  ...
  const importCatalog = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      ...
      const rows = lines.map((row) => Object.fromEntries(row.split(",").map((value, index) => [headers[index], value.trim()])));
      if (headers.includes("stock") && !headers.includes("name")) {
        const updates = rows.filter((row) => row.sku).map((row) => ({ sku: row.sku, stock: Math.max(0, Number(row.stock) || 0) }));
        const updatedProducts = products.map((product) => { const update = updates.find((row) => row.sku === product.sku); return update ? { ...product, stock: update.stock } : product; });
        const changed = updatedProducts.filter((product, index) => product.stock !== products[index].stock);
        if (changed.length) { setProducts(updatedProducts); fetch("/api/products", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ products: changed }) }); notify(`${changed.length} inventory quantities updated`); }
        else notify("No matching SKUs found. Upload the product list before its inventory update.");
      } else {
        const parsed = rows.map((row) => { const existing = products.find((product) => product.sku === row.sku); return { sku: row.sku, name: row.name, category: row.category || "Imported", pack: row.pack || "Each", stock: row.stock === undefined || row.stock === "" ? existing?.stock || 0 : Number(row.stock) || 0, price: Number(row.price) || 0, published: row.published ? !["false", "no", "0"].includes(row.published.toLowerCase()) : true }; }).filter((product) => product.sku && product.name);
        if (parsed.length) { setProducts((current) => [...parsed, ...current.filter((product) => !parsed.some((next) => next.sku === product.sku))]); fetch("/api/products", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ products: parsed }) }); notify(`${parsed.length} products imported`); }
        else notify("No valid product rows found. Download and use the product-list template.");
      }
    };
  ...
  return (
    <div className="app-shell">
      ...
      <main className="admin-main">
        ...
        {view === "orders" && (orderCreated ? <Orders orderCreated={orderCreated} stage={stage} setStage={setStage} products={products} cart={cart} quoteLines={quoteLines} setQuoteLines={setQuoteLines} discountPercent={discountPercent} setDiscountPercent={setDiscountPercent} quoteNotes={quoteNotes} setQuoteNotes={setQuoteNotes} changeRequest={changeRequest} setChangeRequest={setChangeRequest} go={go} notify={notify} /> : <EmptyState title="No orders yet" description="Customer requests will appear here after a secure catalog link is submitted." action="Create an order list" onAction={() => go("create-list")} />)}
        {view === "documents" && (orderCreated ? <Documents stage={stage} setStage={setStage} products={products} cart={cart} quoteLines={quoteLines} discountPercent={discountPercent} quoteNotes={quoteNotes} go={go} notify={notify} /> : <EmptyState title="No estimates yet" description="Create an estimate after a customer submits an order request." action="View orders" onAction={() => go("orders")} />)}
        {view === "customers" && <Customers customers={customers} setCustomers={setCustomers} notify={notify} />}
        {view === "settings" && <Settings notify={notify} />}
      </main>
      {editing && <EditProduct product={editing} onClose={() => setEditing(null)} onDelete={() => { setProducts((all) => all.filter((p) => p.sku !== editing.sku)); setEditing(null); notify("Product deleted"); }} onSave={(updated) => { setProducts((all) => all.map((p) => p.sku === updated.sku ? updated : p)); setEditing(null); notify("Product updated"); }} />}
...
function Products({ products, setProducts, filtered, query, setQuery, categories, category, setCategory, setEditing, importCatalog, notify, go }: { products: Product[]; setProducts: (p: Product[]) => void; filtered: Product[]; query: string; setQuery: (s: string) => void; categories: string[]; category: string; setCategory: (s: string) => void; setEditing: (p: Product | null) => void; importCatalog: (e: ChangeEvent<HTMLInputElement>) => void; notify: (s: string) => void; go: (v: View) => void }) {
  ...
  const deleteProduct = (product: Product) => { if (!window.confirm(`Delete ${product.name} (${product.sku})? This cannot be undone.`)) return; setProducts(products.filter((item) => item.sku !== product.sku)); notify("Product deleted"); };
  const clearProducts = () => { if (!products.length || !window.confirm(`Clear all ${products.length} products and inventory records? This cannot be undone.`)) return; setProducts([]); notify("Product list cleared"); };
  return <div className="page">
    <div className="page-head"><div><p className="eyebrow">Catalog</p><h1>Products & inventory</h1><p>Manage availability, pricing, stock, and customer visibility.</p><div className="template-links"><span>Download templates:</span><a href="/templates/desi-kitchen-product-list.csv" download>Product list CSV</a><a href="/templates/desi-kitchen-inventory.csv" download>Inventory CSV</a></div></div><div className="actions"><label className="button secondary file-button">⇧ Import PDF or CSV<input type="file" accept=".csv,.pdf,text/csv,application/pdf" onChange={importCatalog} /></label><button className="button secondary" onClick={() => notify("Current catalog export prepared")}>⇩ Export</button><button className="button danger-outline" disabled={!products.length} onClick={clearProducts}>Clear all products</button><button className="button primary" onClick={addProduct}>＋ Add product</button></div></div>
...
function Orders({ orderCreated, stage, setStage, products, cart, quoteLines, setQuoteLines, discountPercent, setDiscountPercent, quoteNotes, setQuoteNotes, changeRequest, setChangeRequest, go, notify }: { orderCreated: boolean; stage: Stage; setStage: (s: Stage) => void; products: Product[]; cart: Cart; quoteLines: QuoteLine[]; setQuoteLines: (lines: QuoteLine[]) => void; discountPercent: number; setDiscountPercent: (n: number) => void; quoteNotes: string; setQuoteNotes: (s: string) => void; changeRequest: string; setChangeRequest: (s: string) => void; go: (v: View) => void; notify: (s: string) => void }) {
  ...
  return <div className="page"><div className="page-head"><div><p className="eyebrow">Orders / Current request</p><h1>Customer order request</h1><p>Submitted through a secure customer catalog</p></div><div className="actions"><span className={`badge ${stage}`}>{stage === "request" ? "New request" : stage === "proforma" ? "Estimate" : stage}</span></div></div>
    ...
    <div className="order-footer"><div><small>Final estimate total</small><strong>{money(amounts.total)}</strong></div><button className="button secondary" onClick={() => notify("Estimate draft saved")}>Save draft</button><button className="button primary" disabled={!draftLines.length} onClick={() => { setQuoteLines(draftLines); setChangeRequest(""); setStage("proforma"); go("documents"); notify(changeRequest ? "Revised estimate sent" : "Estimate created and sent"); }}>{changeRequest ? "Send revised estimate →" : "Send for approval →"}</button></div>
  </div>;
}
...
function OrderReview({ cartItems, cart, setQty, setReviewOpen, setOrderCreated }: any) { return <div className="drawer-backdrop"><section className="review-drawer"><div className="drawer-head"><div><p className="eyebrow">Step 2 of 2</p><h2>Review your request</h2></div><button className="close" onClick={() => setReviewOpen(false)}>×</button></div><div className="review-items">{cartItems.map((p: CustomerProduct) => <div className="review-item" key={p.sku}><span><b>{p.name}</b><small>{p.sku} · {p.pack}</small></span><div className="qty-control"><button onClick={() => setQty(p.sku, cart[p.sku] - 1)}>−</button><input value={cart[p.sku]} onChange={(e) => setQty(p.sku, Number(e.target.value))}/><button onClick={() => setQty(p.sku, cart[p.sku] + 1)}>＋</button></div></div>)}</div><div className="form-grid"><label>Contact name<input /></label><label>Business name<input /></label><label>Email<input type="email" /></label><label>Phone<input type="tel" /></label><label>Fulfillment<select><option>Delivery</option><option>Pickup</option></select></label><label>Requested date<input type="date" /></label><label className="full">Delivery address<input /></label><label className="full">Notes<textarea placeholder="Delivery instructions or special requests" /></label></div><div className="notice"><b>This is an order request.</b> Pricing and final availability will be confirmed in your estimate.</div><button className="button customer-cta wide" onClick={() => { setOrderCreated(true); setReviewOpen(false); }}>Submit order request →</button></section></div>; }
...
function Customers({ customers, setCustomers, notify }: { customers: CustomerRecord[]; setCustomers: (records: CustomerRecord[]) => void; notify: (message: string) => void }) {
  ...
  return <div className="page"><div className="page-head"><div><p className="eyebrow">Customer directory</p><h1>Customers</h1><p>Upload customers and issue a unique access code that expires after 24 hours.</p><div className="template-links"><span>Download template:</span><a href="/templates/desi-kitchen-customers.csv" download>Customer data CSV</a></div></div><label className="button primary file-button">⇧ Upload customer CSV<input type="file" accept=".csv,text/csv" onChange={upload}/></label></div><section className="panel"><div className="toolbar"><div className="search"><span>⌕</span><input aria-label="Search customers" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search company, contact, or email"/></div><span className="result-count">{visible.length} customers</span></div><div className="table-wrap"><table><thead><tr><th>Business</th><th>Contact</th><th>Email</th><th>Phone</th><th>Access code</th><th>Valid until</th><th>Action</th></tr></thead><tbody>{visible.map((customer) => <tr key={customer.id}><td><strong>{customer.business}</strong></td><td>{customer.contact}</td><td>{customer.email}</td><td>{customer.phone}</td><td><strong className="access-code">{codeStatus(customer) ? customer.accessCode : "—"}</strong></td><td>{codeStatus(customer) ? new Date(customer.codeExpiresAt!).toLocaleString() : customer.accessCode ? <span className="expired-code">Expired</span> : "Not generated"}</td><td><button className="button secondary code-button" onClick={() => generateCode(customer)}>{codeStatus(customer) ? "Regenerate code" : "Generate code"}</button></td></tr>)}</tbody></table></div></section></div>;
}
