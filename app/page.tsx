...
type View = "landing" | "privacy" | "support" | "fast-quote" | "home" | "products" | "create-list" | "orders" | "order-detail" | "documents" | "customers" | "settings" | "catalog";
...
function pricedQuoteLines(products: Product[], cart: Cart, quoteLines: QuoteLine[]) {
  ...
  return quoteLines.map((line) => {
    ...
    return product
      ? { ...product, ...line }
      : { sku: line.sku, name: `Archived product ${line.sku}`, category: "Archived", pack: "Previously quoted", price: line.unitPrice, stock: 0, published: false, ...line };
  });
}
...
export default function Home() {
  ...
  const customerProducts: CustomerProduct[] = products.filter((p) => {
    const shared = p.published && (!targetedList || targetSkus.includes(p.sku));
    const matchesSearch = `${p.sku} ${p.name}`.toLowerCase().includes(query.toLowerCase());
    const matchesCategory = category === "All products" || p.category === category;
    return shared && matchesSearch && matchesCategory;
  }).sort(compareSkuAscending).map(({ price: _privatePrice, ...product }) => product);
  ...
  if (view === "catalog") return <CustomerCatalog {...{ products: customerProducts, pdfProducts: products, query, setQuery, categories, category, setCategory, cart, setQty, cartItems: customerCartItems, reviewOpen, setReviewOpen, orderCreated, setOrderCreated, stage, setStage, go, notify, targetedList, targetSkus, changeRequest, setChangeRequest, quoteLines, discountPercent, shippingAmount, taxPercent, quoteNotes, orderCustomer, setOrderCustomer, proformaTotal: stage === "proforma" ? quotedTotal : undefined }} />;
    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <nav>
          ...
          <Nav label="Orders" icon="▤" badge={savedOrderCount ? String(savedOrderCount) : undefined} active={view === "orders" || view === "order-detail"} onClick={() => go("orders")} />
      ...
      <main className="admin-main">
        ...
        {view === "create-list" && <SalesOrderListBuilder products={products.filter((p) => p.published)} customers={customers} selectedCustomerId={selectedCustomerId} setSelectedCustomerId={setSelectedCustomerId} selected={targetSkus} setSelected={setTargetSkus} quantities={targetQuantities} setQuantities={setTargetQuantities} onPrerequisite={(next) => go(next)} onSend={(customer, orderId, items) => { const createdAt = new Date().toISOString(); const order: SavedOrder = { id: orderId, customerId: customer.id, customer, items, status: "request", createdAt, updatedAt: createdAt }; setOrderCustomer(customer); setTargetedList(true); setCurrentOrderId(orderId); setCart(Object.fromEntries(items.map((item) => [item.sku, item.quantity]))); setOrderCreated(true); setStage("request"); setSavedOrders((orders) => [order, ...orders.filter((entry) => entry.id !== orderId)]); setSavedOrderCount((count) => count + 1); }} notify={notify} />}
        {view === "orders" && <OrdersInbox orders={savedOrders} openOrder={openSavedOrder} go={go}/>}
        {view === "order-detail" && (orderCreated && stage !== "approved" ? <Orders orderCreated={orderCreated} stage={stage} setStage={setStage} saveOrderStatus={saveOrderStatus} products={products} cart={cart} quoteLines={quoteLines} setQuoteLines={setQuoteLines} discountPercent={discountPercent} setDiscountPercent={setDiscountPercent} shippingAmount={shippingAmount} setShippingAmount={setShippingAmount} taxPercent={taxPercent} setTaxPercent={setTaxPercent} quoteNotes={quoteNotes} setQuoteNotes={setQuoteNotes} changeRequest={changeRequest} setChangeRequest={setChangeRequest} go={go} notify={notify} /> : <EmptyState title="This estimate is already approved" description="Approved estimates can only be viewed as the accepted document." action="Open approved estimate" onAction={() => go("documents")} />)}
...
function Dashboard({ savedOrderCount, savedOrders, openSavedOrder, products, salesUser, go }: { orderCreated: boolean; savedOrderCount: number; savedOrders: SavedOrder[]; openSavedOrder: (order: SavedOrder) => void; stage: Stage; products: Product[]; salesUser: SalesUser | null; go: (v: View) => void; notify: (s: string) => void }) {
  ...
  const awaitingApproval = savedOrders.filter((order) => order.status === "proforma");
  return <div className="page dashboard-page">
    ...
    {awaitingApproval.length > 0 && <section className="panel recent awaiting-list"><div className="panel-head"><div><h2>Awaiting customer approval</h2><p>Open an estimate to review it, record approval, and generate the approved PDF.</p></div><span className="badge proforma">{awaitingApproval.length} pending</span></div>{awaitingApproval.map((order) => <OrderRow key={order.id} initials={(order.customer?.business || "CU").slice(0,2).toUpperCase()} customer={order.customer?.business || "Customer estimate"} number={order.id} details={`${order.items.length} products · sent ${new Date(order.updatedAt || order.createdAt).toLocaleString()}`} status="Awaiting approval" onClick={() => openSavedOrder(order)} />)}</section>}
    <div className="dashboard-grid">
      <section className="panel recent"><div className="panel-head"><div><h2>Recent order requests</h2><p>Latest activity from your customers</p></div><button className="text-button" onClick={() => go("orders")}>View all →</button></div>
        {savedOrders.length ? [...savedOrders].sort((first, second) => ({ proforma: 0, request: 1, approved: 2 }[first.status] - { proforma: 0, request: 1, approved: 2 }[second.status])).slice(0, 8).map((order) => <OrderRow key={order.id} initials={(order.customer?.business || "CU").slice(0,2).toUpperCase()} customer={order.customer?.business || "Customer order"} number={order.id} details={`${order.items.length} products · ${new Date(order.createdAt).toLocaleString()}`} status={order.status === "request" ? "New" : order.status === "proforma" ? "Awaiting approval" : "Approved"} fresh={order.status === "request"} onClick={() => openSavedOrder(order)} />) : <div className="empty inline-empty"><h2>No order requests</h2><p>Send a secure catalog link after uploading customers and products.</p></div>}
      </section>
...
function OrdersInbox({ orders, openOrder, go }: { orders: SavedOrder[]; openOrder: (order: SavedOrder) => void; go: (view: View) => void }) {
  return <div className="page"><div className="page-head"><div><p className="eyebrow">Orders</p><h1>Order requests</h1><p>Open a submitted request to build its estimate. Approved records open as locked documents.</p></div><button className="button primary" onClick={() => go("create-list")}>＋ Create order list</button></div><section className="panel recent"><div className="panel-head"><div><h2>All submitted requests</h2><p>Sales drafts remain in Create Order until a customer submits them.</p></div></div>{orders.length ? orders.map((order) => <OrderRow key={order.id} initials={(order.customer?.business || "CU").slice(0, 2).toUpperCase()} customer={order.customer?.business || "Customer order"} number={order.id} details={`${order.items.length} products · ${new Date(order.updatedAt || order.createdAt).toLocaleString()}`} status={order.status === "request" ? "New" : order.status === "proforma" ? "Quoted" : "Approved"} fresh={order.status === "request"} onClick={() => openOrder(order)}/>) : <div className="empty inline-empty"><h2>No submitted orders</h2><p>Customer submissions will appear here. Unsent sales order lists remain saved as drafts.</p><button className="button secondary" onClick={() => go("create-list")}>Create an order list</button></div>}</section></div>;
}

function SalesOrderListBuilder({ products, customers, selectedCustomerId, setSelectedCustomerId, selected, setSelected, quantities, setQuantities, onPrerequisite, onSend, notify }: { products: Product[]; customers: CustomerRecord[]; selectedCustomerId: string; setSelectedCustomerId: (id: string) => void; selected: string[]; setSelected: (s: string[]) => void; quantities: Cart; setQuantities: (quantities: Cart) => void; onPrerequisite: (view: "customers" | "products") => void; onSend: (customer: CustomerRecord, orderId: string, items: Array<{ sku: string; quantity: number }>) => void; notify: (s: string) => void }) {
  ...
  const categorizedProducts = useMemo(() => products.filter((product) => product.category.trim()), [products]);
  const categoryOptions = useMemo(() => Array.from(new Set(categorizedProducts.map((product) => product.category))).sort((first, second) => first.localeCompare(second)), [categorizedProducts]);
  const categoryProducts = categoryFilter === "all" ? categorizedProducts : categorizedProducts.filter((product) => product.category === categoryFilter);
  const visible = categoryProducts.filter((p) => `${p.sku} ${p.name}`.toLowerCase().includes(search.toLowerCase()));
  const groupedVisible = useMemo(() => Array.from(new Set(visible.map((product) => product.category))).sort((first, second) => first.localeCompare(second)).map((category) => ({ category, products: visible.filter((product) => product.category === category).sort(compareSkuAscending) })), [visible]);
  ...
  const selectCategory = (categoryProducts: Product[]) => { selectionTouched.current = true; const categorySkus = categoryProducts.map((product) => product.sku); const allSelected = categorySkus.every((sku) => selected.includes(sku)); const next = allSelected ? selected.filter((sku) => !categorySkus.includes(sku)) : Array.from(new Set([...selected, ...categorySkus])); setSelected(next); setQuantities(Object.fromEntries(next.map((sku) => [sku, quantities[sku] || 1]))); };
  ...
  const downloadEstimate = () => {
    const estimateProducts = products.filter((product) => selected.includes(product.sku));
    const estimateCart = Object.fromEntries(selected.map((sku) => [sku, quantities[sku] || 1]));
    downloadAwaitingApprovalEstimatePdf(estimateProducts, estimateCart, [], 0, 0, 0, "Prepared by sales and awaiting customer approval.", customer);
    notify(`Awaiting-approval estimate downloaded with ${estimateProducts.length} products`);
  };
  ...
  return <div className="page">
    ...
    <div className="builder-grid"><section className="panel builder-main"><div className="builder-section"><span className="builder-number">1</span><div><h2>Choose customer</h2><p>Search uploaded customers by business, contact, or email.</p></div></div><div className="customer-choice"><span className="avatar pale">{customer?.business.slice(0,2).toUpperCase() || "CU"}</span><span><b>{customer?.business || "Select a customer"}</b><small>{customer ? `${customer.contact} · ${customer.email}` : "Each customer has a separate saved draft"}</small></span><select aria-label="Select customer" value={selectedCustomerId} onChange={(event) => setSelectedCustomerId(event.target.value)}><option value="">Select customer…</option>{customers.map((record) => <option key={record.id} value={record.id}>{record.business} — {record.contact}</option>)}</select></div>
      <div className="builder-section"><span className="builder-number">2</span><div><h2>Select products and starting quantities</h2><p>Choose a category from the uploaded CSV to see only the products in that group. Customers will see selected products without prices and may edit every quantity.</p></div></div><div className="builder-filters"><label className="category-slicer"><span>Group by CSV category</span><select aria-label="Filter products by category" value={categoryFilter} onChange={(event) => { setCategoryFilter(event.target.value); setSearch(""); }}><option value="all">All CSV categories ({categorizedProducts.length})</option>{categoryOptions.map((categoryName) => <option key={categoryName} value={categoryName}>{categoryName} ({categorizedProducts.filter((product) => product.category === categoryName).length})</option>)}</select></label><div className="search builder-search"><span>⌕</span><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder={categoryFilter === "all" ? "Search all products or SKU" : `Search ${categoryFilter}`} /></div></div><div className="category-slicer-status"><span>{categoryFilter === "all" ? "Showing all CSV categories" : `Showing ${categoryFilter}`}</span><b>{visible.length} products</b>{categoryFilter !== "all" && <button type="button" onClick={() => setCategoryFilter("all")}>Clear category filter</button>}</div><div className="actions builder-bulk-actions"><button className="button secondary" onClick={selectAllVisible}>Select all {visible.length} shown products</button><button className="button secondary" disabled={!selected.length} onClick={clearSelection}>Clear selection</button><strong>{selected.length} selected</strong></div><div className="builder-products grouped">{groupedVisible.map((group, groupIndex) => { const selectedCount = group.products.filter((product) => selected.includes(product.sku)).length; const allSelected = selectedCount === group.products.length; return <details className="builder-category" key={group.category} open={categoryFilter !== "all" || Boolean(search) || groupIndex === 0}><summary><span><b>{group.category}</b><small>{group.products.length} products{selectedCount ? ` · ${selectedCount} selected` : ""}</small></span><button type="button" className="category-select" onClick={(event) => { event.preventDefault(); selectCategory(group.products); }}>{allSelected ? "Clear category" : "Select category"}</button></summary><div className="builder-category-products">{group.products.map((p) => <div className="builder-product" key={p.sku}><input aria-label={`Select ${p.name}`} type="checkbox" checked={selected.includes(p.sku)} onChange={() => toggle(p.sku)} /><span><b>{p.name}</b><small>{p.sku} · {p.pack} · {p.stock < 20 ? "Low stock" : "In stock"}</small></span>{selected.includes(p.sku) && <label className="builder-quantity">Starting qty<input aria-label={`Starting quantity for ${p.name}`} type="number" min="1" max={Math.max(1, p.stock)} value={quantities[p.sku] || 1} onChange={(event) => setInitialQuantity(p.sku, Number(event.target.value))} /></label>}</div>)}</div></details>; })}</div>
  ...
  return <div className="page"><div className="page-head"><div><p className="eyebrow">Estimates</p><h1>{approved ? "Approved estimate" : "Customer estimate"}</h1><p>Prepared from the current customer order request</p></div><div className="actions">{approved ? <button className="button primary" onClick={download}>⇩ Download approved estimate PDF</button> : <button className="button primary" onClick={downloadAwaitingApproval}>⇩ Download estimate PDF</button>}<button className="button secondary" onClick={() => { navigator.clipboard?.writeText(location.href); notify("Secure document link copied"); }}>↗ Copy secure link</button></div></div>
    <div className="document-grid"><section className="invoice-sheet"><div className="invoice-top"><div><img className="brand-logo" src="/desi-kitchen-logo.png" alt=""/><h2>Desi Kitchen</h2><p>Paramount, California</p></div><div className="invoice-title"><span className={`badge ${approved ? "approved" : "proforma"}`}>{approved ? "approved" : "awaiting approval"}</span><h2>ESTIMATE</h2><p>Customer estimate</p></div></div><div className="invoice-parties"><div><small>BILL TO</small><strong>Customer on order request</strong><p>Contact and delivery details<br/>are supplied with the secure order.</p></div><div><small>DOCUMENT DETAILS</small><p>Issued when sent by sales<br/>Validity and terms set by sales</p></div></div><table><thead><tr><th>Item</th><th>Qty</th><th>Unit price</th><th>Total</th></tr></thead><tbody>{lines.map((line) => <tr key={line.sku}><td><b>{line.name}</b><small>{line.sku} · {line.pack}</small></td><td>{line.quantity}</td><td>{money(line.unitPrice)}</td><td>{money(line.unitPrice * line.quantity)}</td></tr>)}</tbody></table><div className="invoice-total"><span>Subtotal <b>{money(subtotal)}</b></span>{discountPercent > 0 && <span>Discount ({discountPercent}%) <b>-{money(discount)}</b></span>}<span>Shipping <b>{money(shipping)}</b></span><span>Tax ({taxPercent}%) <b>{money(tax)}</b></span><strong>Total <b>{money(total)}</b></strong></div>{quoteNotes && <p className="invoice-note"><b>Sales note:</b> {quoteNotes}</p>}<p className="invoice-note">{approved ? "Approved by the customer. Pricing, quantities, and terms are accepted." : "Thank you for your business. Customer approval is required."}</p></section>
      <aside className="panel timeline"><div className="panel-head"><div><h2>Approval status</h2><p>This MVP ends with an approved PDF</p></div></div><div className="timeline-step done"><span>✓</span><div><b>Estimate created</b><small>Pricing and stock confirmed</small></div></div><div className={`timeline-step ${approved ? "done" : ""}`}><span>{approved ? "✓" : "2"}</span><div><b>Customer approved</b><small>{approved ? "Completed" : "Pending"}</small></div></div>
        {stage === "proforma" && <><button className="button primary wide" onClick={downloadAwaitingApproval}>Download estimate PDF</button><button className="button secondary wide" onClick={() => go("catalog")}>Open customer approval view</button><button className="button secondary wide" onClick={() => { setStage("approved"); downloadApprovedProformaPdf(products, cart, quoteLines, discountPercent, shippingAmount, taxPercent, quoteNotes, customer); notify("Approval recorded and approved estimate downloaded"); }}>Record approval & download approved PDF</button></>}
...
function CustomerCatalog(props: any) {
  const { products, pdfProducts, query, setQuery, categories, category, setCategory, cart, setQty, cartItems, reviewOpen, setReviewOpen, orderCreated, setOrderCreated, stage, setStage, go, notify, targetedList, targetSkus, changeRequest, setChangeRequest, quoteLines, discountPercent, shippingAmount, taxPercent, quoteNotes, orderCustomer, setOrderCustomer, proformaTotal } = props;
...
function Customers({ customers, setCustomers, notify }: { customers: CustomerRecord[]; setCustomers: (records: CustomerRecord[]) => void; notify: (message: string) => void }) {
  ...
  const upload = (event: ChangeEvent<HTMLInputElement>) => { const file = event.target.files?.[0]; if (!file) return; const reader = new FileReader(); reader.onload = async () => { const rows = String(reader.result).split(/\r?\n/).slice(1).filter(Boolean); const parsed = rows.map((row) => { const [business, contact, email, phone, address] = parseCsvRow(row); return { id: business.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, ""), business, contact, email, phone, address }; }).filter((customer) => customer.business && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(customer.email)); if (!parsed.length) { notify("No valid customers found. Use: business,contact,email,phone,address"); return; } try { const response = await fetch("/api/customers", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ customers: parsed }) }); if (!response.ok) throw new Error("Customer import failed"); setCustomers([...parsed, ...customers.filter((customer) => !parsed.some((next) => next.id === customer.id))]); notify(`${parsed.length} customers saved permanently`); } catch { notify("Customer upload could not be saved. No customer records were changed."); } }; reader.readAsText(file); event.target.value = ""; };
  const generateCode = async (customer: CustomerRecord) => { const code = Array.from(crypto.getRandomValues(new Uint8Array(6)), (value) => "ABCDEFGHJKLMNPQRSTUVWXYZ23456789"[value % 32]).join(""); const codeExpiresAt = new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString(); const updated = customers.map((record) => record.id === customer.id ? { ...record, accessCode: code, codeExpiresAt } : record); try { const response = await fetch("/api/customers", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ customers: [updated.find((record) => record.id === customer.id)] }) }); if (!response.ok) throw new Error("Access code save failed"); setCustomers(updated); navigator.clipboard?.writeText(code); notify(`24-hour code ${code} generated, saved, and copied`); } catch { notify("Access code could not be saved. Please try again."); } };
  const codeStatus = (customer: CustomerRecord) => customer.codeExpiresAt && new Date(customer.codeExpiresAt).getTime() > Date.now();
  const saveCustomer = async (customer: CustomerRecord) => { const exists = customers.some((record) => record.id === customer.id); try { const response = await fetch("/api/customers", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ customers: [customer] }) }); if (!response.ok) throw new Error("Customer save failed"); const updated = exists ? customers.map((record) => record.id === customer.id ? customer : record) : [customer, ...customers]; setCustomers(updated); setEditingCustomer(null); notify(exists ? "Customer updated and saved" : "Customer added and saved"); } catch { notify("Customer could not be saved. Please try again."); } };
  const deleteCustomer = async (customer: CustomerRecord) => { if (!window.confirm(`Delete ${customer.business}? This also invalidates its access code.`)) return; try { const response = await fetch("/api/customers", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: customer.id }) }); if (!response.ok) throw new Error("Customer delete failed"); setCustomers(customers.filter((record) => record.id !== customer.id)); setEditingCustomer(null); notify("Customer permanently deleted"); } catch { notify("Customer could not be deleted. No records were changed."); } };
