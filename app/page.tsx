...
function createProformaPdf(products: Product[], cart: Cart, quoteLines: QuoteLine[], discountPercent: number, quoteNotes: string, approved: boolean) {
  ...
  commands.push(
    ...
    rule(36, 55, 576, 55), text(approved ? "Approved pro-forma. Pricing and quantities accepted by Sample Grocery." : "Preview only. Customer approval is required before this pro-forma is accepted.", 36, 36, 8, false, ".35 .40 .38"),
...
function Orders({ orderCreated, stage, setStage, products, cart, quoteLines, setQuoteLines, discountPercent, setDiscountPercent, quoteNotes, setQuoteNotes, changeRequest, setChangeRequest, go, notify }: { orderCreated: boolean; stage: Stage; setStage: (s: Stage) => void; products: Product[]; cart: Cart; quoteLines: QuoteLine[]; setQuoteLines: (lines: QuoteLine[]) => void; discountPercent: number; setDiscountPercent: (n: number) => void; quoteNotes: string; setQuoteNotes: (s: string) => void; changeRequest: string; setChangeRequest: (s: string) => void; go: (v: View) => void; notify: (s: string) => void }) {
  ...
  return <div className="page"><div className="page-head"><div><p className="eyebrow">Orders / OR-2026-0137</p><h1>{orderCreated ? "Sample Grocery" : "Demo Market"}</h1><p>Customer request · received today at 2:18 PM</p></div><div className="actions"><span className={`badge ${stage}`}>{stage === "request" ? "New request" : stage}</span></div></div>
    ...
    <div className="order-footer"><div><small>Final pro-forma total</small><strong>{money(amounts.total)}</strong></div><button className="button secondary" onClick={() => notify("Pro-forma draft saved")}>Save draft</button><button className="button primary" disabled={!draftLines.length} onClick={() => { setQuoteLines(draftLines); setChangeRequest(""); setStage("proforma"); go("documents"); notify(changeRequest ? "Revised pro-forma PF-2026-0042 sent" : "Pro-forma PF-2026-0042 created"); }}>{changeRequest ? "Send revised pro-forma →" : "Send for approval →"}</button></div>
  </div>;
}
...
function CustomerCatalog(props: any) {
  ...
  if (changeRequest) return <div className="customer-shell centered"><header className="customer-header"><Brand /><CustomerRep /></header><main className="confirmation"><span className="success-ring">✓</span><p className="eyebrow">Changes requested</p><h1>Your note was sent to Sahil.</h1><p>Sales will revise <b>PF-2026-0042</b> and send you an updated pro-forma for approval.</p><div className="change-summary"><small>Your request</small><p>{changeRequest}</p></div><button className="button primary" onClick={() => go("orders")}>Open sales review →</button></main></div>;
  if (orderCreated && !reviewOpen) return <div className="customer-shell centered"><header className="customer-header"><Brand /><CustomerRep /></header><main className="confirmation"><span className="success-ring">✓</span><p className="eyebrow">Request received</p><h1>Thank you, Maya.</h1><p>Your order request <b>OR-2026-0137</b> has been sent to Sahil. You’ll receive a priced pro-forma after stock is reviewed.</p><div className="confirmation-card"><span>{cartItems.length} product lines</span><span>Delivery requested Aug 12</span><span>No payment is due yet</span></div><p className="rep-help">Questions? Call Sahil directly at <a href="tel:+15626336329">(562) 633-6329</a>.</p><button className="button primary" onClick={() => go("home")}>Open sales workspace →</button></main></div>;
